        ...
        return this.request(this.zone, 'getStatus')
            .then(response => {
                return {
                    ...
                    sound_program: response.sound_program ? response.sound_program : false
                };
            });
    }

    request(zone, action) {
        console.log('http://' + this.ipAddress + '/YamahaExtendedControl/v1/' + zone + '/' + action);
        return axios
